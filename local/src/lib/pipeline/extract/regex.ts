...
function parseTransactionLine(line: string, currency: string): ParsedTransaction | null {
  ...
  if (!date || parsedAmount === null || !merchant || /balance/i.test(merchant)) {
    return null;
  }

  const isPayment = parsedAmount.isCredit || looksLikePayment(merchant);
...
export async function checkRegexExtractionAvailability(): Promise<ProviderAvailability> {
  return { available: true, message: "Built-in regex parser ready." };
}

export async function extractWithRegex(input: ExtractionInput): Promise<ParsedTransaction[]> {
  ...
  for (const line of lines) {
    const transaction = parseTransactionLine(line, input.currency);
